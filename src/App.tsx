      ...
      <div className="absolute inset-0 p-12 pointer-events-none">
        <AnimatePresence>
          {(step === 'glyphHovered' || step.includes('node') || step === 'revealed') && (
            <TriangleNode position="top-12 left-12" isActive={step !== 'glyphHovered'} id={1} onClick={() => setStep('node1')} />
          )}
          {(step === 'node1' || step.includes('node2') || step === 'revealed') && (
            <TriangleNode position="top-12 right-12" isActive={step === 'node2' || step === 'node3' || step === 'revealed'} id={2} onClick={() => setStep('node2')} />
          )}
          {(step === 'node2' || step === 'revealed') && (
            <TriangleNode position="bottom-12 right-12" isActive={step === 'revealed'} id={3} onClick={() => setStep('revealed')} />
          )}
        </AnimatePresence>
      </div>
